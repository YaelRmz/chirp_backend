package com.yrmz.chirp.infra.rate_limiting

import com.yrmz.chirp.infra.config.NginxConfig
...
class IpResolver(
    ...
    private fun extractFromXRealIp(
        ...
        return request.getHeader("X-Real-IP")?.let { header ->
            validateAndNormalizedIp(header, "X-Real-IP", proxyIp)
        }
    }

    private fun validateAndNormalizedIp(ip: String, headerName: String, proxyIp: String): String? {
        ...
        return try {
            val inetAddr = when {
                ...
                trimmedIP.matches(Regex("\\d+\\.\\d+\\.\\d+\\.\\d+")) -> Inet4Address.getByName(
                    trimmedIP
                )
