...
import com.yrmz.chirp.domain.exception.InvalidDeviceTokenException
import com.yrmz.chirp.domain.model.DeviceToken
import com.yrmz.chirp.domain.model.PushNotification
...
import com.yrmz.chirp.infra.database.DeviceTokenRepository
import com.yrmz.chirp.infra.mappers.toDeviceToken
import com.yrmz.chirp.infra.mappers.toPlatformEntity
import com.yrmz.chirp.infra.push_notification.FirebasePushNotificationService
...
class PushNotificationService(
    ...
    fun sendWithRetry(
        ...
        if (result.succeeded.isNotEmpty()) {
            logger.info("Successfully sent notifications to ${result.succeeded.size} devices")
        }
    }
    ...
    private fun scheduledRetry(
        ...
        val executedAtMillis = executeAt.toEpochMilli()
        ...
        retryQueue.compute(executedAtMillis) { _, retries ->
        ...
        entries.forEach { (timeMillis, retries) ->
            ...
            retries.forEach { retry ->
                try {
                    ...
                    logger.warn("Error processing retry ${retry.notification}", e)
                }
            }
        }
    }
